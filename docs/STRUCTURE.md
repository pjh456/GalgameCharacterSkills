...
模块包含以下子模块：
- `result` — 统一结果封装 `Result[T]`
- `catch` — 异常到 Result 的映射装饰器
- `validate` — 字典字段校验器
- `paths` — 工作区路径布局
- `executors` — 线程池管理与 `@to_async` 装饰器
...
- 检查文件和目录的存在性
- 在线程池中提供 `a` 前缀异步方法 (`aread` / `awrite` / `aappend`)，供异步调用方使用
...
模块包含以下子模块：
- `client` — 网络请求客户端门面，持有配置并转发执行
- `errors` — 网络错误结果构造
- `executor` — 同步与异步 HTTP 请求发送与重试
- `models` — `HttpMethod`、`HttpResponse`、`JsonResponse` 等数据模型
- `request` — 请求体构造、URL 拼接、请求头合并
- `response` — 原始响应到内部模型的转换与 JSON 解析
- `retry` — 重试判定与指数退避
...
- 提供面向调试与排错的日志查询接口
- 写入并读取日志文件
- 在线程池中提供 `a` 前缀异步方法 (`awrite` / `aread` / `aquery`)，供异步调用方使用

模块包含以下子模块：
- `models` — `LogRecord` 日志记录模型
- `logger` — 统一日志接口，级别过滤与重试
- `writer` — 日志写入器，线程安全
- `reader` — 日志读取与查询器
- `errors` — 日志模块错误结果构造
